...
<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1>Products</h1>
        <div>
            @can('create', '\DryRun\Product\Models\Product') {{-- Check create permission --}}
            <a href="{{ route('product.products.create') }}" class="btn btn-primary me-2">
                 <i class="fas fa-plus me-1"></i> Add New Product
            </a>
            @endcan
            {{-- Link to view trashed items --}}
            @if(Route::has('product.products.trashed'))
                <a href="{{ route('product.products.trashed') }}" class="btn btn-outline-secondary">
...
@push('scripts')
    ...
    $(document).ready(function() {
        const productTable = $('#product-table').DataTable({
            ...
            ajax: "{{ $isTrashed ?? false ? route('product.products.trashed') : route('product.products.index') }}", // Adjust AJAX source for trashed view
            columns: [
                { data: 'id', name: 'id' },
                @php
...
@can('view', ${{$item}})
    <a href="{{ route('product.products.show', ${{$item}}->id) }}" class="btn btn-sm btn-info me-1" title="View">
...
@if(isset(${{$item}}->deleted_at) && ${{$item}}->deleted_at) {{-- Item is soft deleted --}}
    @can('restore', ${{$item}})
        <form action="{{ route('product.products.restore', ${{$item}}->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to restore this product?');">
            @csrf
    ...
    @can('forceDelete', ${{$item}})
        <form action="{{ route('product.products.forceDelete', ${{$item}}->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to permanently delete this product? This action cannot be undone.');">
            @csrf
            @method('DELETE')
...
@else {{-- Item is not soft deleted --}}
    @can('update', ${{$item}})
        <a href="{{ route('product.products.edit', ${{$item}}->id) }}" class="btn btn-sm btn-warning me-1" title="Edit">
            <i class="fas fa-edit"></i>
        </a>
    @endcan
    @can('delete', ${{$item}})
        <form action="{{ route('product.products.destroy', ${{$item}}->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete (soft) this product?');">
            @csrf
            @method('DELETE')
...
`.replace(/\{\{itemVar\}\}/g, 'row'); // Placeholder for actual action stub content
                        ...
                        let viewUrl = "{{ route('product.products.show', ':id') }}".replace(':id', row.id);
                        let editUrl = "{{ route('product.products.edit', ':id') }}".replace(':id', row.id);
                        let destroyUrl = "{{ route('product.products.destroy', ':id') }}".replace(':id', row.id);

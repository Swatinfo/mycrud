...
@section('content')
    <div class="container-fluid mt-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1>Products</h1>
            <div>
                @can('create', 'Modules\Products\Models\Product')
                    ...
                    <table id="product-table" class="table table-striped table-hover table-bordered" style="width:100%">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th>Is Store Room</th>
                                <th>Unit Id</th>
                                <th>Rack Place</th> {{-- This will be populated by the command --}}
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody></tbody>
    ...
    <script>
        $(document).ready(function() {
            const productTable = $('#product-table').DataTable({
                ...
                serverSide: true,
                responsive: true,
                ajax: "{{ $isTrashed ?? false ? route('products.products.trashed') : route('products.products.index') }}",
                /*columns: [
                    ...
                    { // Actions column
                        ...
                        render: function(data, type, row) {
                            ...
                            let viewUrlTemplate = "{{ route('products.products.show', ':id') }}";
                            let editUrlTemplate = "{{ route('products.products.edit', ':id') }}";
                            let destroyUrlTemplate =
                                "{{ route('products.products.destroy', ':id') }}";
                            let restoreUrlTemplate =
                                "{{ route('products.products.restore', ':id') }}";
                            let forceDeleteUrlTemplate =
                                "{{ route('products.products.forceDelete', ':id') }}";
                            ...
                            // View button
                            actions +=
                                `<a href="${viewUrl}" class="btn btn-sm btn-info me-1" title="View"><i class="fas fa-eye"></i></a>`;

                            if (row.deleted_at) { // Check if 'deleted_at' is present and truthy
                                // Restore Action
                                actions +=
                                    `<form action="${restoreUrl}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to restore this product?');">`;
                                actions += csrfFieldHtml;
                                actions +=
                                    `<button type="submit" class="btn btn-sm btn-success me-1" title="Restore"><i class="fas fa-undo"></i></button></form>`;

                                // Force Delete Action
                                actions +=
                                    `<form action="${forceDeleteUrl}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to permanently delete this product? This action cannot be undone.');">`;
                                actions += csrfFieldHtml;
                                actions += methodDeleteFieldHtml;
                                actions +=
                                    `<button type="submit" class="btn btn-sm btn-danger" title="Force Delete"><i class="fas fa-trash-alt"></i></button></form>`;
                            } else {
                                // Edit Action
                                actions +=
                                    `<a href="${editUrl}" class="btn btn-sm btn-warning me-1" title="Edit"><i class="fas fa-edit"></i></a>`;

                                // Soft Delete Action
                                actions +=
                                    `<form action="${destroyUrl}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete (soft) this product?');">`;
                                actions += csrfFieldHtml;
                                actions += methodDeleteFieldHtml;
                                actions +=
                                    `<button type="submit" class="btn btn-sm btn-danger" title="Delete (Soft)"><i class="fas fa-trash"></i></button></form>`;
                            }
                            return actions;
                        }
                    }
